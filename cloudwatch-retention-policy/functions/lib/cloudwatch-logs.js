  ...
  const loop = async (nextToken, acc = []) => {
    ...
    try {
      const resp = await retry(
        ...
        getRetryConfig((err) =>
          log.warn("retrying describeLogGroups after error...", { req }, err)
        )
      );
      ...
      log.error(
        `failed to fetch log groups, processing the fetched groups [${acc.length}] so far`,
        error
      );
      return acc;
    }
...
const setExpiry = async (logGroupName) => {
  ...
  await retry(
    ...
    getRetryConfig((err) =>
      log.warn(
        "retrying putRetentionPolicy after error...",
        { logGroupName },
        err
      )
    )
    ...
    .then(() =>
      log.debug(`${logGroupName}: retention policy updated`, { logGroupName })
    )
    .catch((err) =>
      log.error(
        `${logGroupName}: failed to update retention policy, skipped...`,
        { logGroupName },
        err
      )
    );
